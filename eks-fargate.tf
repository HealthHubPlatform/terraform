...
data "aws_eks_cluster" "cluster" {
  name = aws_eks_cluster.healthhubplatform.id
}
...
resource "aws_cloudwatch_log_group" "eks_cluster" {
  name              = "/aws/eks/${var.eks_cluster_name}-${var.environment}/cluster"
  retention_in_days = 30

  tags = {
    Name        = "${var.eks_cluster_name}-${var.environment}-eks-cloudwatch-log-group"
    Environment = var.environment
  }
}

resource "aws_eks_cluster" "healthhubplatform" {
  ...
  vpc_config {
    subnet_ids = concat(data.terraform_remote_state.global.outputs.healthhubplatform_vpc_public_subnets, data.terraform_remote_state.global.outputs.healthhubplatform_vpc_private_subnets)
  }
...
data "external" "thumbprint" {
  ...
  depends_on = [aws_eks_cluster.healthhubplatform]
}

resource "aws_iam_openid_connect_provider" "healthhubplatform" {
...
resource "aws_eks_node_group" "healthhubplatform" {
  cluster_name    = aws_eks_cluster.healthhubplatform.name
  ...
  tags = {
    Name        = "${var.eks_cluster_name}-${var.environment}-eks-node-group"
    Environment = var.environment
  }
...
data "template_file" "kubeconfig" {
  ...
  vars = {
    kubeconfig_name     = "eks_${aws_eks_cluster.healthhubplatform.name}"
    clustername         = aws_eks_cluster.healthhubplatform.name
